...
pub struct StreamingParser<D: DemoStream, V: Visitor> {
    demo_stream: D,
    buf: Vec<u8>,
    visitor: V,
    ctx: Context,
...
impl<D: DemoStream, V: Visitor> StreamingParser<D, V> {
        ...
        loop {
            match self.demo_stream.read_cmd_header() {
                ...
                Err(err) => {
                    if matches!(
                        err,
                        super::demostream::ReadCmdHeaderError::IoError(ref e)
...
pub struct Parser<D: SeekableDemoStream, V: Visitor> {
    inner: StreamingParser<D, V>,
}

impl<D: SeekableDemoStream, V: Visitor> Parser<D, V> {
    pub fn from_stream_with_visitor(demo_stream: D, visitor: V) -> Result<Self, DemoHeaderError> {
        Ok(Self {
            inner: StreamingParser::from_stream_with_visitor(demo_stream, visitor)?,
        })
    }
    ...
    {
        loop {
            match self.inner.demo_stream.read_cmd_header() {
                Ok(cmd_header) => {
                    self.inner.ctx.prev_tick = self.inner.ctx.tick;
                    self.inner.ctx.tick = cmd_header.tick;
                    match handler(self, &cmd_header).await? {
                        ControlFlow::HandleCmd => {
                            self.inner.handle_cmd(&cmd_header).await?;
                            if self.inner.ctx.prev_tick != self.inner.ctx.tick {
                                self.inner.visitor.on_tick_end(&self.inner.ctx).await?;
                            }
                        }
                        ControlFlow::SkipCmd => self.inner.demo_stream.skip_cmd(&cmd_header)?,
                        ControlFlow::IgnoreCmd => {}
                        ControlFlow::Break => {
                            self.inner.demo_stream.unread_cmd_header(&cmd_header)?;
                            self.inner.ctx.tick = self.inner.ctx.prev_tick;
                            return Ok(());
                        }
                    }
                }
                Err(err) => {
                    if self.inner.demo_stream.is_at_eof().unwrap_or_default() {
                        return Ok(());
                    }
                    return Err(err.into());
                }
    ...
    pub async fn run_to_tick(&mut self, target_tick: i32) -> anyhow::Result<()> {
        ...
        self.run(
            async |notnotself: &mut Parser<D, V>, cmd_header: &CmdHeader| {
                ...
                let distance_to_target_tick = target_tick - notnotself.inner.ctx.tick;
                ...
                let has_full_packet_ahead =
                    distance_to_target_tick > notnotself.inner.ctx.full_packet_interval + 100;
                if is_full_packet {
